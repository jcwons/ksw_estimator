...
cdef local_shape(np.ndarray[double,ndim=4] beta_s, long long ell1, long long ell2, long long ell3, int pidx1, int pidx2, int pidx3, np.ndarray[double,ndim=1] shape_func):
    cdef int l_min = 2
    # bba
    shape_func =  beta_s[ell1 - l_min, pidx1, 1, :] * beta_s[ell2 - l_min, pidx2, 1, :] * beta_s[ell3 - l_min, pidx3, 0, :]
    # bab
    shape_func += beta_s[ell1 - l_min, pidx1, 1, :] * beta_s[ell2 - l_min, pidx2, 0, :] * beta_s[ell3 - l_min, pidx3, 1, :]
    # abb
    shape_func += beta_s[ell1 - l_min, pidx1, 0, :] * beta_s[ell2 - l_min, pidx2, 1, :] * beta_s[ell3 - l_min, pidx3, 1, :]
    # local shape has prefactor 2
    shape_func = 2 * shape_func
    return shape_func

cdef equil_shape(np.ndarray[double,ndim=4] beta_s, long long ell1, long long ell2, long long ell3, int pidx1, int pidx2, int pidx3, np.ndarray[double,ndim=1] shape_func):
    cdef int l_min = 2
    #bba,bab,abb
    shape_func = -beta_s[ell1 - l_min, pidx1, 1, :] * beta_s[ell2 - l_min, pidx2, 1, :] * beta_s[ell3 - l_min, pidx3, 0, :] 
    shape_func -= beta_s[ell1 - l_min, pidx1, 1, :] * beta_s[ell2 - l_min, pidx2, 0, :] * beta_s[ell3 - l_min, pidx3, 1, :] 
    shape_func -= beta_s[ell1 - l_min, pidx1, 0, :] * beta_s[ell2 - l_min, pidx2, 1, :] * beta_s[ell3 - l_min, pidx3, 1, :]
    # delta delta delta
    shape_func -= 2 * beta_s[ell1 - l_min, pidx1, 3, :] * beta_s[ell2 - l_min, pidx2, 3, :] * beta_s[ell3 - l_min, pidx3, 3, :]
    # beta gamma delta
    ...
    # equilateral shape has prefactor 6
...
def compute_bispec(ells, radii, beta_s, invcov, fisher_sub, pol_trpl, rank, shape, lmin):
    ...
    if shape == 'local':
        shape_temp = 0
        if rank == 0: 
            print('calculate local shape')
    else:
        shape_temp = 1
        if rank == 0: 
            print('calculate equilateral shape')
    compute_bispec_worker(ells, radii, r2, beta_s, invcov, shape_func, fisher_sub, pol_trpl, rank, shape_temp, lmin)
    return fisher_sub
...
cdef compute_bispec_worker(np.ndarray[long long,ndim=1] ells, np.ndarray[double,ndim=1] radii, np.ndarray[double,ndim=1] r2, np.ndarray[double,ndim=4] beta_s, \
                           np.ndarray[double,ndim=3] invcls, np.ndarray[double,ndim=1] shape_func, np.ndarray[double,ndim=1] fisher_sub, np.ndarray[long,ndim=2] pol_trpl, \
                           long rank, long shape_temp, int lmin):
    ...
    for ell3 in ells:
        fisher = 0 
        if rank == 0: 
            print('lmax:', lidx3+lmin)#, flush=True)
        for ell2 in range(lmin, ell3 + 1):
            for ell1 in range(lmin, ell2 + 1):
                ...
                for pidx3, pidx2, pidx1 in pol_trpl:
                    ...
                    if shape_temp == 0:
                        shape_func = local_shape(beta_s, ell1, ell2, ell3, pidx1, pidx2, pidx3, shape_func)
                    else:
                        shape_func = equil_shape(beta_s, ell1, ell2, ell3, pidx1, pidx2, pidx3, shape_func)
                    ...
                    for pidx6, pidx5, pidx4 in pol_trpl:
                        if (pidx3, pidx2, pidx1) == ( pidx6, pidx5, pidx4):
                            ...
                            if shape_temp == 0:
                                shape_func = local_shape(beta_s, ell1, ell2, ell3, pidx4, pidx5, pidx6, shape_func)
                            else:
                                shape_func = equil_shape(beta_s, ell1, ell2, ell3, pidx4, pidx5, pidx6, shape_func)
