...
def get_transfer_bin_d(Delta_bin, z_bin, k, ells, data):
    ...
    for zidx, z in enumerate(z_rank):
        ...
        for lidx, ell in enumerate(ells):
            ...
            delta_v = Dv_Tk[:] * bessel
            delta_d_lk[lidx, :] += delta_v
    return delta_d_lk

def run_camb(lmax=300, N_bins=4,pol='both'):
    ...
    pkl_file = open('Output/transfer_delta_class.pkl', 'rb')
    ...
    for i in range(N_bins):
        ...
        if pol == 'velocity':
            ...
            delta_v_zlk /= z_bin.size
        elif pol == 'density':
            Delta_bin = deltas[:, start:end]
            delta_d_zlk[i, :, :] = get_transfer_bin_v(Delta_bin, z_bin, k, ells, data)
    ...
    tag = './Output/class_300_b_{}.pkl'.format(N_bins)
    if MPI_rank == 0:
...
run_camb(lmax=300, N_bins=N_bins,pol=pol)
